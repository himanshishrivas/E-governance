...
from .models import resource_manage
#from django.http import HttpResponseRedirect
#from django.urls import reverse
from django.views import generic
 
class IndexView(generic.TemplateView):
    template_name = 'water_management/index.html'
...
def saveInsurInfo(request):
    ...
    return render(request, 'gas_supply/index.html')
...
class ModifyView(generic.TemplateView):
    template_name = 'water_management/modify.html'

class Modify2View(generic.TemplateView):
    template_name = 'water_management/modify2.html'
    
def readData(request):
    if request.session.get('member_id'):
        insinfo = resource_manage.objects.all() \
            .filter(user_name__exact = request.session.get('member_id')).values();
        if insinfo.count() == 0:
            return render(request, "water_management/index.html");
        else:
            if insinfo.count() == 1:
                return render(request, "water_management/modify.html", {"insinfo_list": insinfo,})
            else: 
                return render(request, "water_management/modify2.html",
...
def modifyInsurInfo(request):
    if request.session.get('member_id'):
        pi = resource_manage.objects.all() \
            .filter(user_name__exact = request.session.get('member_id')).values();
        if pi.count() == 2:
            pi0 = resource_manage.objects.get(pk=request.POST['pkPrimary'])
            ...
            pi1 = resource_manage.objects.get(pk=request.POST['pkAdditional'])
