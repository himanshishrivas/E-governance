...
from .models import electricity_supply
...
class Meds(Resource):
    ...
    def get(self, request):
        if request.session.get('member_id'):
            meds = electricity_supply.objects.all() \
    ...
    def put(self, request, patientmeds_id):
        if request.session.get('member_id'):
            meds = electricity_supply.objects.get(pk = patientmeds_id, user_name=request.session.get('member_id'))
    ...
    def delete(self, request, patientmeds_id):
        meds = electricity_supply.objects.get(pk = patientmeds_id)
        meds.delete()
        return HttpResponse(status = 200)
